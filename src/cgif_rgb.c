...
static colHashTable* init_col_hash_table(uint32_t tableSize){
  ...
  if(!colhash->frequ || !colhash->hashTable || !colhash->indexUsed || !colhash->pPalette || !colhash->colIdx) {
    free(colhash->frequ); free(colhash->hashTable); free(colhash->indexUsed);
    free(colhash->pPalette); free(colhash->colIdx); free(colhash);
    return NULL;
  }
...
static void resize_col_hash_table(colHashTable* colhash){
  ...
  if(!pPalette_new || !colIdx_new || !hashTable_new || !indexUsed_new || !frequ_new) {
    free(hashTable_new); free(indexUsed_new); free(frequ_new);
    return; // original colhash unchanged; caller can keep using it
  }
...
cgif_result cgif_rgb_addframe(CGIFrgb* pGIF, const CGIFrgb_FrameConfig* pConfig) {
  ...
  if(pNewBef == NULL) { pGIF->curResult = CGIF_ERROR; return CGIF_ERROR; }
  ...
  if(fConfig.pImageData == NULL) { free(pNewBef); pGIF->curResult = CGIF_ERROR; return CGIF_ERROR; }
  ...
  if(colhash == NULL) {
    free(pNewBef); free(fConfig.pImageData);
    pGIF->curResult = CGIF_ERROR;
    return CGIF_ERROR;
  }
